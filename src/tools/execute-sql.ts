...
export const ExecuteSqlInputSchema = z.object({
  session_id: z.string().optional().describe("Session ID from connect (optional if DATABASE_URL header set)"),
  sql: z.string().min(1).describe("SELECT query to execute — generated by the AI client"),
  database_url: z.string().optional().describe("Postgres URL — reconnects on serverless if session expired"),
})
